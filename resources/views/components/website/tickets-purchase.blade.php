<div class="d-flex col-12 justify-content-center align-items-center">
	<form action = "#" class="container-responsive">
		...
		<div class="form-group row">
			...
			<label for="ticketDate" class="col-xl-4 col-form-label text-secondary text-left">{{ __('Select ticket date') }}</label>
		...
		<table class="table table-responsive">
			...
			<tbody>
				@foreach($data as $ticket)
					<tr>
						<td scope="row" class="text-center text-secondary">{{$ticket->type}}</td>
						<td scope="row" class="text-center text-secondary">{{$ticket->price_gate}}</td>
						<td scope="row" class="text-center text-secondary">{{$ticket->price_online}}</td>
